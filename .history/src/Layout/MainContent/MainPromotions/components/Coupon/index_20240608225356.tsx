import "./index.css";
import "../../../../../styles/global.css";

interface Item {
  ship: string;
  priceOrder: string;
  price: string;
  code: string;
  date: string;
}

const Coupon = ({ ship, priceOrder, code, date, price }: Item) => {
  return (
    <>
      <div id="MainPromotions__coupon-item" className="flex flex-row bg-[#fff]">
        <div className="flex items-center justify-center basis-1/4 ">
          <span className="text-[--bgflashsale] text-[22px]">{price}</span>
        </div>
        <div className="p-[10px] basis-1/2">
          <span className="text-black text-[15px] font-bold inline-block">
            {ship}
          </span>
          <span className="text-[11px] font-normal text-black">
            {priceOrder}
          </span>
          <span className="text-[11px] text-black inline-block">{code}</span>
          <span className="text-[11px] text-black inline-block">{date}</span>
        </div>
      </div>
    </>
  );
};

export default Coupon;
